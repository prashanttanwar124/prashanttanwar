  ...
  <div class>
    <div class="uk-flex-middle uk-margin-medium-bottom" uk-grid>
      ...
      <div class="uk-width-1-2 uk-flex-first">
        <div>
          <h4 class="uk-h4 uk-margin-remove">Review Table</h4>
        </div>
      </div>
    </div>
      ...
      <vs-table notFound>
        ...
        <template #thead>
          <vs-tr>
            ...
            <vs-th>Name</vs-th>
            <vs-th>Client Image</vs-th>
            <vs-th>User Post</vs-th>
        ...
        <template #tbody>
          <vs-tr :key="i" v-for="(tr, i) in $vs.getPage(
            ...
            <vs-td>{{ tr.name }}</vs-td>
            <vs-td>
              <img :src="tr.client_image" @click="viewImage(tr.client_image)" style="width:80px" alt="client_image" />
            </vs-td>
            <vs-td>{{ tr.user_post }}</vs-td>
    ...
    <vs-dialog overflow-hidden not-center v-model="activeModel">
      ...
      <div class="uk-card-body uk-margin-small-top uk-width-large uk-margin-medium-top uk-padding-remove-top uk-padding-remove-bottom">
        <vs-input v-model="form.name" label="User Name" class="labelYes" :readonly="deleteBtn">
        ...
        <div class="uk-margin-medium-top uk-width-1-1@m">
          <vs-input v-model="form.user_post" class="labelYes" label="User Post" :readonly="deleteBtn">
            <template #message-danger>
              <has-error :form="form" field="user_post"></has-error>
            </template>
          </vs-input>
        </div>
        ...
        <div class="uk-margin-medium-top uk-width-1-1">
          <vs-input v-model="form.message" class="labelYes" label="Message" :readonly="deleteBtn">
            <template #message-danger>
        ...
        <div class="uk-margin" v-if="!deleteBtn">
          <span class="uk-text-middle">Select Client Image</span>
          <div uk-form-custom>
          ...
          <div class="uk-text-small uk-text-danger">
            <has-error :form="form" field="client_image"></has-error>
          </div>

          <img style="margin-top:6px" :src="form.client_image" alt v-if="form.client_image !== '' && !editBtn" />
        </div>
        ...
        <div class="uk-margin uk-margin-medium-top" v-if="editBtn">
          ...
          <img :src="form.client_image" alt v-if="editBtn" />
        </div>
      </div>
  ...
  methods: {
    ...
    imageSubmit(e) {
      ...
      if (e.target.files[0] !== null) {
        ...
        reader.onloadend = () => {
          this.form.client_image = reader.result;
        };
      }
    },
    viewImage(value) {
    ...
    deleteData() {
      this.form
        .delete(`/data/review/${this.form.id}`)
        .then(res => {
          this.form.reset();
          this.activeModel = false;
          this.getData();
