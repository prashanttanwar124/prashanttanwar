  ...
  <div class>
    <div class="uk-flex-middle uk-margin-medium-bottom" uk-grid>
      ...
      <div class="uk-width-1-2 uk-flex-first">
        <div>
          <h4 class="uk-h4 uk-margin-remove">Team Table</h4>
        </div>
      </div>
    </div>
      ...
      <vs-table notFound>
        ...
        <template #thead>
          <vs-tr>
            ...
            <vs-th>Name</vs-th>
            <vs-th>Member Post</vs-th>
            <vs-th>Member Image</vs-th>
        ...
        <template #tbody>
          <vs-tr :key="i" v-for="(tr, i) in $vs.getPage(
            ...
            <vs-td>{{ tr.id }}</vs-td>
            <vs-td>{{ tr.name }}</vs-td>
            <vs-td>{{ tr.member_post  }}</vs-td>
            <vs-td>
              <img :src="tr.member_image" @click="viewImage(tr.member_image)" style="width:80px" alt="member_image" />
            </vs-td>
    ...
    <vs-dialog overflow-hidden not-center v-model="activeModel">
      ...
      <div class="uk-card-body uk-margin-small-top uk-width-large uk-margin-medium-top uk-padding-remove-top uk-padding-remove-bottom">
        <vs-input v-model="form.name" class="labelYes" label="Member Name" :readonly="deleteBtn">
        ...
        <div class="uk-margin-medium-top">
          <vs-input v-model="form.member_post" class="labelYes" label="Member Post" :readonly="deleteBtn">
            <template #message-danger>
              <has-error :form="form" field="member_post"></has-error>
            </template>
          </vs-input>
        </div>
        ...
        <div class="uk-margin" v-if="!deleteBtn">
          ...
          <div class="uk-text-small uk-text-danger">
            <has-error :form="form" field="member_image"></has-error>
          </div>

          <img style="margin-top:6px" :src="form.member_image" alt v-if="form.member_image !== '' && !editBtn" />
        </div>
        ...
        <div class="uk-margin uk-margin-medium-top" v-if="editBtn">
          ...
          <img :src="form.member_image" alt v-if="editBtn" />
        </div>
      </div>
  ...
  methods: {
    ...
    imageSubmit(e) {
      ...
      if (e.target.files[0] !== null) {
        ...
        reader.onloadend = () => {
          this.form.member_image = reader.result;
        };
      }
    },
    viewImage(value) {
